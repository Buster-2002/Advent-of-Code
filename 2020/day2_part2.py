...
def main() -> int:
    ...
    for combo in INPUT.splitlines():
        ...
        password = combo[2]
        if (
            (password[(_range[0] - 1)] == letter
            and not password[(_range[1] - 1)] == letter
            ) or (
            password[(_range[1] - 1)] == letter 
            and not password[(_range[0] - 1)] == letter)
        ):
            correct += 1
...
if __name__ == '__main__':
    result = main()
    print(f'Answer day 2, part 2: {result}')
